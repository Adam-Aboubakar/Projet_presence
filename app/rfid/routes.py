...
def revoquer_carte(carte_id):
    ...
    if raison not in ['perdu', 'vole', 'defectueux', 'autre']:
        return jsonify({
            'succes': False,
            'message': 'Raison invalide. Valeurs: perdu, vole, defectueux, autre'
        }), 400
...
def remplacer_carte(personne_id):
    ...
    return jsonify({
        'succes': True,
        'message': 'Carte remplacée avec succès',
        'carte_id': nouvelle_carte.id
    }), 201


# ============================================================
# CAS 4 — Vérifier une carte (Raspberry Pi)
# ============================================================
...
def verifier_carte():
    ...
    for carte in cartes_actives:
        try:
            if dechiffrer_texte(carte.numero_rfid) == numero_rfid:
                personne = Personne.query.get(carte.personne_id)
                if personne and personne.est_actif:  # ← corrigé : est_actif
                    return jsonify({
                        'autorise':   True,
                    ...
                    journaliser('carte_personne_inactive',
                                "Tentative avec carte d'une personne inactive",
                                severite='WARNING')
                    db.session.commit()
    ...
    journaliser('carte_inconnue', 'Tentative avec carte inconnue', severite='CRITIQUE')
    db.session.commit()
    return jsonify({'autorise': False, 'message': 'Carte inconnue'}), 403


# ============================================================
# GET — Historique des cartes d'une personne (API mobile)
# ============================================================
@rfid_bp.route('/api/<string:personne_id>/liste', methods=['GET'])
@role_requis('agent')
def liste_cartes(personne_id):
    Personne.query.get_or_404(personne_id)
    cartes = CarteRFID.query.filter_by(personne_id=personne_id)\
        .order_by(CarteRFID.attribuee_le.desc()).all()
...
# ============================================================
# PAGE WEB — Liste des cartes RFID
# ============================================================
@rfid_bp.route('/')
@role_requis('agent')
def liste():
